...
7. Examine main-deadlock-global.c.  
   ...
   4. What does this tell you about tools like helgrind?  
      1. This shows that helgrind can report fals positives. Even if the program is logically thread-safe, helgrind may still report a “lock order violation” because it flags potential risks regardless of whether a deadlock is possible.  
...
9. Run helgrind on this program.  
   ...
   2. Is the code correct?  
      1. Yes the code is logically correct but inefficient. Even though helgrind flags it, the parent thread is spinning until the child thread updates the value. There is no risk of memory corruption or inconsistent state that would break the program’s logic.   
10. Now look at the slightly modified version of the code found in main-signal-cv.c. This version uses a condition variable to do the signaling (and associated lock).  
    ...
    2. Is it correctness, or performance, or both?  
       1. The improvements is mainly for performance. By using a condition variable, the parent thread is put to sleep by the OS instead of being busy waiting, freeing up the CPU to perform other tasks.   
